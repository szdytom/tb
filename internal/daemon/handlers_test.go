...
func TestCreateAndGetBuffer(t *testing.T) {
	...
	// Get
	resp = sendRequest(t, conn, ipc.NewRequest(2, ipc.OpGetBuffer, ipc.IDPayload(idResp)))
	if !resp.Ok {
...
func TestUpdateContent(t *testing.T) {
	...
	resp = sendRequest(t, conn, ipc.NewRequest(3, ipc.OpGetBuffer, ipc.IDPayload(idResp)))
...
func TestUpdateLabelAndTags(t *testing.T) {
	...
	sendRequest(t, conn, ipc.NewRequest(3, ipc.OpUpdateTags,
		ipc.UpdateTagsPayload{ID: idResp.ID, Tags: []string{"x", "y"}}))

	resp = sendRequest(t, conn, ipc.NewRequest(4, ipc.OpGetBuffer, ipc.IDPayload(idResp)))
...
func TestSoftDeleteAndRestore(t *testing.T) {
	...
	if err := resp.UnmarshalPayload(&trash); err != nil {
		t.Fatal(err)
	}
...
func TestPermanentlyDelete(t *testing.T) {
	...
	resp = sendRequest(t, conn, ipc.NewRequest(2, ipc.OpPermanentlyDelete, ipc.IDPayload(idResp)))
	if !resp.Ok {
...
func TestSearch(t *testing.T) {
	...
	sendRequest(t, conn, ipc.NewRequest(1, ipc.OpCreateBuffer,
		ipc.CreateBufferPayload{Content: "error: timeout occurred"}))
